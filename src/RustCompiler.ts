...
export class RustCompiler {
  ...
  compile_env_pos = (x: string): { pos: [number, number]; meta: VarMeta } => {
    ...
    return {
      pos: [frame_index, valInd],
      meta: this.compile_env[frame_index][valInd],
    };
  };

  value_index = (frame: VarMeta[], x: string) => {
    for (let i = 0; i < frame.length; i++) if (frame[i].sym === x) return i;
    return -1;
  };
  ...
  scan_for_vars = (comp: compile_comp): VarMeta[] =>
    comp.tag === "seq"
      ? (comp.stmts as compile_comp[]).reduce(
          (acc: VarMeta[], x) => acc.concat(this.scan_for_vars(x)),
          []
        )
      : comp.tag === "fun"
      ? [
        ...
        ? [
            {
              ...
              mut_ref: 0,
              point_to: "",
              is_copy: !(comp.type as string).includes("mut"),
            },
          ] // NOTE: Treat all as mutable can be reassigned
        : [
            {
              sym: comp.sym,
              dec: false,
              ...
              imm_ref: 0,
              mut_ref: 0,
              is_copy: primitive_types.includes(comp.type as string),
            },
          ]
      : [];
  ...
  compile_comp = {
    ...
    unop: (comp: { sym: string; frst: compile_comp }) => {
      ...
      if (op === "&" || op === "&mut" || op === "*unary") {
        ...
        if (op === "&mut") {
          if (!meta.is_mut)
            throw new Error(
              `Cannot apply &mut to ${frst.sym}, only mutable lvalue allowed`
            );
          if (meta.imm_ref || meta.mut_ref)
            throw new Error(
              `Cannot apply &mut to ${frst.sym}, already borrowed`
            );
        }
      ...
      for (let arg of comp.args) {
        ...
        if (res && res.tag === "tempref") {
          ...
          if (res.is_mut && (meta.imm_ref || meta.mut_ref))
            throw new Error(
              `Cannot obtain mutable reference to ${res.var} as argument, already borrowed`
            );
    ...
    assmt:
      ...
      (comp: { expr: compile_comp; sym: string }) => {
        const { pos, meta } = this.compile_env_pos(comp.sym);
        if (!meta.is_mut || meta.imm_ref || meta.mut_ref)
          throw new Error(
            `Cannot assign to ${comp.sym} if it is not mutable or is borrowed`
          );
        ...
        if (meta.point_to !== undefined) {
          if (!meta.is_mut)
            throw new Error(
              `Cannot reassign immutable ${comp.sym} to ${res.var}`
            );
          ...
          if (comp.expr.tag === "nam") {
            const { pos: opos, meta: ometa } = this.compile_env_pos(
              comp.expr.sym as string
            );
            meta.point_to = ometa.point_to;
            if (meta.is_mut) ometa.point_to = null;
            else {
              const { pos: tpos, meta: tmeta } = this.compile_env_pos(
                meta.point_to
              );
              tmeta.imm_ref++;
            }
            ...
            if (npt.meta.mut_ref)
              throw new Error(
                `Cannot assign ${comp.sym} to reference ${res.var} which is already borrowed mutably`
              );
            if (meta.is_mut && (npt.meta.imm_ref || npt.meta.mut_ref))
              throw new Error(
                `Cannot obtain mutable reference to ${res.var} for ${comp.sym}, already borrowed`
              );
            // Check owner has longer lifetime than reference
            // console.log(`npt: ${npt.pos}, ogpt: ${ogpt.pos}, pos: ${pos}`);
            if (npt.pos[0] > pos[0])
              throw new Error(
                `Cannot assign ${comp.sym} to reference ${meta.point_to} which has a shorter lifetime`
              );
    ...
    lam: (comp: {
      ...
      const goto_instruction = { tag: "GOTO", addr: undefined };
      this.instrs[this.wc++] = goto_instruction;
      const prmsInfo: VarMeta[] = comp.prms.map((x) => ({
        sym: x.name,
        dec: true,
        is_mut: primitive_types.includes(x.type) || x.type.includes("mut"),
        imm_ref: 0,
        mut_ref: 0,
        is_copy: primitive_types.includes(x.type) || x.type.includes("&"),
      }));
    ...
    blk: (comp: { body: compile_comp }) => {
      ...
      for (let local of locals)
        if (local.point_to) {
          const { pos: ppos, meta: pmeta } = this.compile_env_pos(
            local.point_to
          );
    ...
    let: (comp: { expr: compile_comp; sym: string; type: string }) => {
      ...
      if (meta.point_to !== undefined) {
        if (comp.expr.tag === "nam") {
          ...
          if (meta.is_mut && (pmeta.imm_ref || pmeta.mut_ref))
            throw new Error(
              `Cannot obtain mutable reference to ${res.var} for ${comp.sym}, already borrowed`
            );
          meta.point_to = res.var;
    ...
    continue: (comp: compile_comp) => {
      const goto_instruction = { tag: "GOTO", addr: undefined };
      const curr_while = this.while_instrs[this.while_instrs.length - 1];
      if (!this.continue_map.has(curr_while))
        this.continue_map.set(curr_while, [goto_instruction]);
